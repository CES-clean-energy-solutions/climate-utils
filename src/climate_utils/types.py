...
class EPWDataFrame(TypedDict):
    ...
    This TypedDict defines the expected column names and types for EPW weather data
    after processing through the climate-utils package.
    """
...
EPWDataFramePsychrometric = TypedDict('EPWDataFramePsychrometric', {
    'Dry Bulb Temperature (°C)': pd.Series[float],
    'Relative Humidity (%)': pd.Series[float],
    'Atmospheric Station Pressure (Pa)': pd.Series[float],
})
